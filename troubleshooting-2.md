...
``` r
### FIXED ERROR HERE ###
#changed load.packages to library and added "install.packages" for dslabs 
#install.packages("dslabs") #Remove if already have package
library(dslabs)
library(tidyverse)
```
...
``` r
### FIXED ERROR HERE ###
# changed dim function to head to be able to see the tibble
...
``` r
### FIXED ERROR HERE ### 
# In fourth set of conditions, changed text "month == 2000" to "year == 2000"
...
``` r
### FIXED ERROR HERE ###
# in select() command rearranged column names to be (title, genres, everything())
movieLens %>%
  filter(!genres == "Drama") %>%
...
``` r
### FIXED ERROR HERE ###
# changed double equals sign (==) to single equals sign (=)
# used head function to check work
movieLens <- movieLens %>%
...
``` r
### FIXED ERROR HERE ### 
# used transmute in place of mutate()
transmute(movieLens,
...
``` r
### FIXED ERROR HERE ###
#added group_by(title), changed mutate() to summarise()
movieLens %>%
  group_by(title) %>%
...
``` r
### FIXED ERROR HERE ###
# combined "height" and "mass" into a character vector to apply the function to both columns 
starWars %>%
  group_by(species) %>%
...
``` r
### FIXED ERROR HERE ###
# added quotation marks around birth locations 
